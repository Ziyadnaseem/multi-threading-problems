...
{
    int evmid = evm->id;
    highlight_green_ln(booth_id(booth->id) + evm_id(evmid) + " called polling_ready_evm() for slot allocation");
    pthread_mutex_lock(&(booth->evm_mutex));
    while (true)
    {
        highlight_green_ln(booth_id(booth->id) + evm_id(evmid) + " will try to fill slots now");
        pthread_mutex_lock(&(booth->registration_mutex));
        highlight_green_ln(booth_id(booth->id) + evm_id(evmid) + " Checking if any voters arrived [ Registered voters: " + to_string(booth->registrations) + " ] [ Voters not registered : " + to_string(booth->unregistered_voters) + " ]");
        if (booth->registrations == 0 && booth->unregistered_voters == 0)
        {
            highlight_green_ln(booth_id(booth->id) + evm_id(evmid) + " All voters already assigned slots. Returning...");
        ...
        else if (booth->registrations >= count || (booth->unregistered_voters == 0 && booth->registrations < count))
        {
            ...
            highlight_green_ln(booth_id(booth->id) + evm_id(evmid) + " Sufficient voters arrived... waking all voters and releasing tokens: " + to_string(evm->tokens));
            ...
            highlight_green_ln(booth_id(booth->id) + evm_id(evmid) + " All slots have been allocated. Returning...");
        ...
        {
            highlight_green_ln(booth_id(booth->id) + evm_id(evmid) + " Sleeping and waiting for sufficient voters to arrive");
            pthread_cond_wait(&(booth->evm_to_voter), &(booth->registration_mutex));
            pthread_mutex_unlock(&(booth->registration_mutex));
            highlight_green_ln(booth_id(booth->id) + evm_id(evmid) + " Woken from sleep to check voters at booth");
        }
    }
}
/**
...
void voter_wait_for_evm(Booth *booth, Voter *voter)
{
    ...
    pthread_mutex_lock(&(booth->registration_mutex));
    highlight_yellow_ln(booth_id(booth->id) + voter_id(voterid) + " arrived at voter_wait_for_evm() for slot allocation");
    ...
    while (true)
    {
        if (booth->selected_evm)
        {
            if (booth->selected_evm->tokens != 0)
            {
                ...
                highlight_yellow_ln(booth_id(booth->id) + voter_id(voterid) + " Token Acquired at " + evm_id(booth->selected_evm->id) + " moving towards evm...");
                highlight_green_ln(booth_id(booth->id) + evm_id(booth->selected_evm->id) + " Tokens left: " + to_string(booth->selected_evm->tokens));
                if (booth->selected_evm->tokens == 0)
            ...
            {
                highlight_yellow_ln(booth_id(booth->id) + voter_id(voterid) + " could not acquire token");
            }
        }
        // wake a single evm and ask it to see if sufficient no. of voters have arrived
        highlight_yellow_ln(booth_id(booth->id) + voter_id(voterid) + " waking an evm to check voter arrival");
        pthread_cond_signal(&booth->evm_to_voter);
        //wait for evm to say that it is accepting slots
        highlight_yellow_ln(booth_id(booth->id) + voter_id(voterid) + " Sleeping and waiting for evm to call");
        pthread_cond_wait(&(booth->voter_to_evm), &(booth->registration_mutex));
        highlight_yellow_ln(booth_id(booth->id) + voter_id(voterid) + " Woken up by Evm and trying to acquire token");
        pthread_mutex_unlock(&(booth->registration_mutex));
        pthread_mutex_lock(&(booth->registration_mutex));
    }
}
...
void voter_in_slot(Booth *booth, Voter *voter) // this is where voting will happen
{
    ...
    highlight_yellow_ln(booth_id(booth->id) + voter_id(voter->id) + " Awoken by " + evm_id(evm->id) + " to cast vote");
    evm->vote_tokens--;
    ...
    pthread_cond_signal(&evm->evm_condition);
    highlight_yellow_ln(booth_id(booth->id) + voter_id(voter->id) + " has casted its vote at " + evm_id(evm->id));
    pthread_mutex_unlock(&(evm->evm_mutex));
}
/***************************************************************************************************************************/
/*                                                                THREADS                                                  */
...
void *evm_thread(void *arg)
{
    ...
    highlight_green_ln(booth_id(booth->id) + evm_id(evm->id) + " is starting up...");
    while (true)
    {
        evm->status = 0;
        ...
        evm->vote_tokens = 0;
        highlight_green_ln(booth_id(booth->id) + evm_id(evm->id) + " Slots free in this cycle : [ " + to_string(evm->slots) + " ]");
        polling_ready_evm(booth, evm->slots, evm);
        pthread_mutex_lock(&(booth->votes_mutex));
        if (booth->total_voters == booth->casted_votes || !evm->status)
        {
            highlight_green_ln(booth_id(booth->id) + evm_id(evm->id) + " is shutting down...");
            pthread_mutex_unlock(&(booth->votes_mutex));
            break;
        }
        pthread_mutex_unlock(&(booth->votes_mutex));
        ...
        while (true)
        {
            ...
            if (evm->vote_tokens == evm->slots)
            {
                ...
                highlight_green_ln(booth_id(booth->id) + evm_id(evm->id) + " Waking all voters to cast their votes");
                pthread_mutex_unlock(&(evm->evm_mutex));
                break;
            }
            highlight_green_ln(booth_id(booth->id) + evm_id(evm->id) + " Sleeping and waiting for all voters to arrive at slots");
            pthread_cond_wait(&(evm->evm_condition), &(evm->evm_mutex));
            highlight_green_ln(booth_id(booth->id) + evm_id(evm->id) + " woken from sleep to check if all voters arrived at slots");
            pthread_mutex_unlock(&(evm->evm_mutex));
        }
        ...
        highlight_green_ln(booth_id(booth->id) + evm_id(evm->id) + " has completed voting cycle");
    }
}
...
void *booth_thread(void *arg)
{
    ...
    highlight_cyan_ln(booth_id(booth->id) + " Booth is starting up...");
